...
def fix_json_dict(output_dir):
    ...
    with open(os.path.join(output_dir, "fixed_da.json"), "w", encoding="utf-8") as f_out:
        for key, value in data.items():
            ...
            f_out.write("\n")
        print(f"File {file_path} has been fixed and saved to {output_dir} as fixed_da.json")
    print("Removing the original xyz.jsons file")
    os.remove(file_path)
...
if __name__ == "__main__":
    if len(sys.argv) !=2:
        ...
        print("Exame Usage")
        print("python3 execute.py /home/shashwat/etl/extract/")
    else:
        try:
