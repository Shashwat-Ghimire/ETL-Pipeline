...
def create_query_tables(output_dir, artists_df, recommendations_df, tracks_df):
    ...
    # tracks exploded with artists array
    tracks_exploded = tracks_df.withColumn("id_artists_array", F.from_json(F.col("id_artists"), F.ArrayType(F.StringType()))) \
    ...
    # tracks metadata
    tracks_metadata = tracks_df.select("id", "name", "popularity", "duration_ms", "danceability", "energy", "tempo")
